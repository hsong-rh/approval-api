RSpec.describe Api::V1x0::WorkflowsController, :type => :request do
  ...
  describe 'POST /templates/:template_id/workflows' do
    ...
    let(:group) { instance_double(Group, :name => 'group', :uuid => 990, :approver_role? => true) }
    ...
    context 'when a request with invalid group' do
      before do
        admin_access
        allow(group).to receive(:approver_role?).and_return(false)
      end
  ...
  describe 'PATCH /workflows/:id' do
    ...
    let(:group) { instance_double(Group, :name => 'n1000', :uuid => '1000', :approver_role? => true) }
